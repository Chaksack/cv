<template>
  <NuxtLink
    ...
    :to="internalTo"
    @mouseenter="$emit('hover', post.title)"
  ...
  >
    <div class="relative border-l border-white/10 pl-8 md:pl-20 py-8 md:py-12 flex flex-col h-full justify-center">
      ...
      <div v-if="externalHref" class="mt-12">
        <span class="font-mono text-[9px] tracking-[0.3em] uppercase text-neutral-400">
          Read ↗
        </span>
      </div>
...
const internalTo = computed(() => (props.post.slug ? `/blog/${props.post.slug}` : null))
const externalHref = computed(() => {
  const href = props.post.link
  if (!href) return null
  if (href === '#') return null
  const isExternal = /^(https?:)?\/\//.test(href)
  return isExternal ? href : null
})
